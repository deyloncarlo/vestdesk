...
public class ModeloVestuarioResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new modeloVestuarioDTO, or with status 400 (Bad Request) if the modeloVestuario has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/modelo-vestuarios")
    @Timed
    public ResponseEntity<ModeloVestuarioDTO> createModeloVestuario(@Valid @RequestBody ModeloVestuarioDTO modeloVestuarioDTO) throws URISyntaxException {
        log.debug("REST request to save ModeloVestuario : {}", modeloVestuarioDTO);
        if (modeloVestuarioDTO.getId() != null) {
            throw new BadRequestAlertException("A new modeloVestuario cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ModeloVestuarioDTO result = modeloVestuarioService.save(modeloVestuarioDTO);
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated modeloVestuarioDTO,
     * or with status 400 (Bad Request) if the modeloVestuarioDTO is not valid,
     * or with status 500 (Internal Server Error) if the modeloVestuarioDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/modelo-vestuarios")
    @Timed
    public ResponseEntity<ModeloVestuarioDTO> updateModeloVestuario(@Valid @RequestBody ModeloVestuarioDTO modeloVestuarioDTO) throws URISyntaxException {
        log.debug("REST request to update ModeloVestuario : {}", modeloVestuarioDTO);
        if (modeloVestuarioDTO.getId() == null) {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of modeloVestuarios in body
     */
    @GetMapping("/modelo-vestuarios")
    @Timed
    public ResponseEntity<List<ModeloVestuarioDTO>> getAllModeloVestuarios(Pageable pageable) {
        log.debug("REST request to get a page of ModeloVestuarios");
        Page<ModeloVestuarioDTO> page = modeloVestuarioService.findAll(pageable);
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the modeloVestuarioDTO, or with status 404 (Not Found)
     */
    @GetMapping("/modelo-vestuarios/{id}")
    @Timed
    public ResponseEntity<ModeloVestuarioDTO> getModeloVestuario(@PathVariable Long id) {
        log.debug("REST request to get ModeloVestuario : {}", id);
        ModeloVestuarioDTO modeloVestuarioDTO = modeloVestuarioService.findOne(id);
    ...
    public ResponseEntity<Void> deleteModeloVestuario(@PathVariable Long id) {
        log.debug("REST request to delete ModeloVestuario : {}", id);
        modeloVestuarioService.delete(id);
