...
public class ConfiguracaoProdutoResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new configuracaoProdutoDTO, or with status 400 (Bad Request) if the configuracaoProduto has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/configuracao-produtos")
    @Timed
    public ResponseEntity<ConfiguracaoProdutoDTO> createConfiguracaoProduto(@Valid @RequestBody ConfiguracaoProdutoDTO configuracaoProdutoDTO) throws URISyntaxException {
        log.debug("REST request to save ConfiguracaoProduto : {}", configuracaoProdutoDTO);
        if (configuracaoProdutoDTO.getId() != null) {
            throw new BadRequestAlertException("A new configuracaoProduto cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ConfiguracaoProdutoDTO result = configuracaoProdutoService.save(configuracaoProdutoDTO);
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated configuracaoProdutoDTO,
     * or with status 400 (Bad Request) if the configuracaoProdutoDTO is not valid,
     * or with status 500 (Internal Server Error) if the configuracaoProdutoDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/configuracao-produtos")
    @Timed
    public ResponseEntity<ConfiguracaoProdutoDTO> updateConfiguracaoProduto(@Valid @RequestBody ConfiguracaoProdutoDTO configuracaoProdutoDTO) throws URISyntaxException {
        log.debug("REST request to update ConfiguracaoProduto : {}", configuracaoProdutoDTO);
        if (configuracaoProdutoDTO.getId() == null) {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of configuracaoProdutos in body
     */
    ...
    public ResponseEntity<List<ConfiguracaoProdutoDTO>> getAllConfiguracaoProdutos(Pageable pageable) {
        log.debug("REST request to get a page of ConfiguracaoProdutos");
        Page<ConfiguracaoProdutoDTO> page = configuracaoProdutoService.findAll(pageable);
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the configuracaoProdutoDTO, or with status 404 (Not Found)
     */
    ...
    public ResponseEntity<ConfiguracaoProdutoDTO> getConfiguracaoProduto(@PathVariable Long id) {
        log.debug("REST request to get ConfiguracaoProduto : {}", id);
        ConfiguracaoProdutoDTO configuracaoProdutoDTO = configuracaoProdutoService.findOne(id);
    ...
    public ResponseEntity<Void> deleteConfiguracaoProduto(@PathVariable Long id) {
        log.debug("REST request to delete ConfiguracaoProduto : {}", id);
        configuracaoProdutoService.delete(id);
